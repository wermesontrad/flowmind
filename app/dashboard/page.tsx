...
export default function Dashboard() {
  ...
  async function criarProjeto() {
    ...
    try {
      const res = await fetch("/api/projetos", {
        ...
        body: JSON.stringify({ nome: nomeProjeto, descricao: descProjeto }),
      });
      const data = await res.json();
      if (data.projeto) {
  ...
  async function moverTarefa(id: string, novoStatus: string) {
    await fetch("/api/tarefas", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id, status: novoStatus }) });
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#f5f5f5", fontFamily: "system-ui, sans-serif" }}>
      ...
      <div style={{ display: "flex", height: "calc(100vh - 56px)" }}>
        <div style={{ width: 220, background: "white", borderRight: "1px solid #eee", padding: 16, display: "flex", flexDirection: "column", gap: 4 }}>
                  ...
                  return (
                    <div key={col.id} style={{ background: "#f0f0f0", borderRadius: 12, padding: 12 }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12 }}>
                        ...
                        <span style={{ marginLeft: "auto", fontSize: 11, color: "#aaa", background: "white", padding: "1px 7px", borderRadius: 10 }}>{tarefasCol.length}</span>
                      </div>
                      {tarefasCol.length === 0 && <div style={{ textAlign: "center", padding: "20px 0", color: "#bbb", fontSize: 12 }}>Nenhuma tarefa</div>}
                      {tarefasCol.map(t => (
      ...
      {showNovoProjeto && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.4)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 100 }} onClick={() => setShowNovoProjeto(false)}>
          <div style={{ background: "white", borderRadius: 16, padding: 28, width: 400, maxWidth: "90vw" }} onClick={e => e.stopPropagation()}>
      ...
      {showNovaTarefa && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.4)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 100 }} onClick={() => setShowNovaTarefa(false)}>
          <div style={{ background: "white", borderRadius: 16, padding: 28, width: 400, maxWidth: "90vw" }} onClick={e => e.stopPropagation()}>
            <div style={{ fontSize: 18, fontWeight: 700, color: "#1a1a1a", marginBottom: 18 }}>Nova tarefa</div>
